...
- Create backend app structure inside backend/.
...
- Framework choice can be selected by the agent if it integrates cleanly with the monorepo and tests.
- Keep a fixture/mock mode available by default.
...
- Run `curl -sS http://localhost:<port>/health` and capture response.
- Run `curl -sS http://localhost:<port>/ready` if readiness endpoint exists.

## Required evidence to version and attach to the PR

- evidence/M3-T01/backend-tests.log.
- evidence/M3-T01/curl-health.json.
- evidence/M3-T01/curl-ready.json or readiness-not-applicable.md.
